...
def aws_sentinel_chip(item):
    ...
    # mosaic_rgb['rgb'] = mosaic_rgb.iloc[0].apply(lambda x: [scale_values(x['rgb'][:,:,0]),scale_values(x['rgb'][:,:,1]),scale_values(x['rgb'][:,:,2])], axis=1)
...
if show_mosaic == 1:
    st.image(mosaic_rgb, clamp=True, channels='BGR')
